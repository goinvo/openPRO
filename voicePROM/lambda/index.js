...
const REPROMPT_TO_CONTINUE = 'what else?';
...
const SleepReport = {
  ...
  handle(handlerInput){
    ...
    const duration = request.intent.slots.sleep_duration.value;
    if (!duration){
      return handlerInput.responseBuilder.addDelegateDirective().getResponse();
    }
    else{
      attributes.sleepduration = duration;
      const attributesManager = handlerInput.attributesManager;
      attributesManager.setSessionAttributes(attributes);
...
const SomeMedicationsTaken = {
  ...
  handle(handlerInput){
    ...
    if (!medA){
      ...
      attributes.medstaken.push(medA);
      if (medB) attributes.medstaken.push(medB);
      if (medC) attributes.medstaken.push(medC);
      const attributesManager = handlerInput.attributesManager;
      attributesManager.setSessionAttributes(attributes);
...
function SendData(handlerInput, status){
  const attributes = getAttributes(handlerInput);
  attributes.status = status;
  // only take unique drugs
  let uniqueDrugs = [...new Set(attributes.medstaken)]; 
  attributes.medstaken = uniqueDrugs.slice();
...
const ExitHandler = {
  ...
  handle(handlerInput) {
    ...
    // Mail results here!!!
    SendData(handlerInput, 'graceful');
